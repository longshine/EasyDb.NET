...
namespace Dapper
{
    ...
    static partial class SqlMapper
    {
        ...
        private static void PurgeQueryCacheByType(Type type)
        {
            ...
            {
#if NET20
                var toRemove = Enumerable.ToArray(Enumerable.Where(_queryCache.Keys, id => id.type == type));
#else
...
#pragma warning restore 618
        ...
        internal static DbType LookupDbType(Type type, string name, bool demand, out ITypeHandler handler)
        {
            ...
            if (TypeExtensions.IsEnum(type) && !typeMap.ContainsKey(type))
            {
...
this 
#endif
IDbConnection cnn, string sql, object param, CommandType commandType)
        {
...
this
#endif
IDbConnection cnn, string sql, object param, IDbTransaction transaction, CommandType commandType)
        {
...
this
#endif
IDbConnection cnn, string sql, object param, IDbTransaction transaction, int? commandTimeout, CommandType? commandType
#else
...
IDbConnection cnn, CommandDefinition command)
        ...
        private static int ExecuteImpl(
#if !NET20
this
#endif
IDbConnection cnn, ref CommandDefinition command)
        {
...
this
#endif
IDbConnection cnn, CommandDefinition command, CommandBehavior commandBehavior)
        {
            IDbCommand dbcmd;
...
this
#endif
IDbConnection cnn, string sql, object param, IDbTransaction transaction, bool buffered, int? commandTimeout, CommandType? commandType)
        {
...
#if NET20
            return command.Buffered ? Enumerable.ToList(data) : data;
#else
...
this
#endif
IDbConnection cnn, Type type, string sql, object param, IDbTransaction transaction, bool buffered, int? commandTimeout, CommandType? commandType
#else
...
IDbConnection cnn, CommandDefinition command)
        ...
        private static GridReader QueryMultipleImpl(
#if !NET20
this
#endif
IDbConnection cnn, ref CommandDefinition command)
...
this
#endif
IDbConnection cnn, CommandDefinition command, Type effectiveType)
        {
            object param = command.Parameters;
...
this
#endif
IDbConnection cnn, string sql, Func<TFirst, TSecond, TReturn> map, object param, IDbTransaction transaction, bool buffered, string splitOn, int? commandTimeout, CommandType? commandType
#else
...
this
#endif
IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TReturn> map, object param, IDbTransaction transaction, bool buffered, string splitOn, int? commandTimeout, CommandType? commandType
#else
...
this
#endif
IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, object param, IDbTransaction transaction, bool buffered, string splitOn, int? commandTimeout, CommandType? commandType
#else
...
this
#endif
IDbConnection cnn, string sql, Delegate map, object param, IDbTransaction transaction, bool buffered, string splitOn, int? commandTimeout, CommandType? commandType)
        {
            var command = new CommandDefinition(sql, (object)param, transaction, commandTimeout, commandType, buffered ? CommandFlags.Buffered : CommandFlags.None);
...
#if NET20
            return buffered ? Enumerable.ToList(results) : results;
#else
            return buffered ? results.ToList() : results;
#endif
        }

        static IEnumerable<TReturn> MultiMapImpl<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TSeventh, TReturn>(
#if !NET20
this
#endif
IDbConnection cnn, CommandDefinition command, Delegate map, string splitOn, IDataReader reader, Identity identity, bool finalize)
        {
            object param = command.Parameters;
...
#if NET20
                    otherDeserializers = cinfo.OtherDeserializers = Enumerable.ToArray(Enumerable.Skip(deserializers, 1));
#else
        ...
        static IEnumerable<TReturn> MultiMapImpl<TReturn>(
#if !NET20
this
#endif
IDbConnection cnn, CommandDefinition command, Type[] types, Func<object[], TReturn> map, string splitOn, IDataReader reader, Identity identity, bool finalize)
        {
            if (types.Length < 1)
            {
...
#if NET20
            var splits = Enumerable.ToArray(Enumerable.Select(splitOn.Split(','), s => s.Trim()));
#else
            var splits = splitOn.Split(',').Select(s => s.Trim()).ToArray();
#endif
                bool isMultiSplit = splits.Length > 1;
#if NET20
            if (Enumerable.First(types) == typeof(Object))
#else
        ...
        internal static Func<IDataReader, object> GetDeserializer(Type type, IDataReader reader, int startBound, int length, bool returnNullIfFirstMissing)
        {
            ...
            if (!(typeMap.ContainsKey(type) || TypeExtensions.IsEnum(type) || type.FullName == LinqBinary ||
                (TypeExtensions.IsValueType(type) && (underlyingType = Nullable.GetUnderlyingType(type)) != null && TypeExtensions.IsEnum(underlyingType))))
            {
                ITypeHandler handler;
                if (typeHandlers.TryGetValue(type, out handler))
                {
...
#region Implementation of IDictionary<string,object>
...
#endif
        internal static Exception MultiMapException(IDataRecord reader)
        {
        ...
        public static void PackListParameters(IDbCommand command, string namePrefix, object value)
        {
            ...
            else
            {
                ...
                {
                    command.CommandText = Regex.Replace(command.CommandText, regexIncludingUnknown,
#if NET20
...
#if NET20
                            return __ToStringRecycle(sb.Append(')'));
#else
                            return sb.Append(')').__ToStringRecycle();
#endif
                        }
        ...
        {
#if NET20
            return Enumerable.Where(parameters, p => Regex.IsMatch(sql, @"[?@:]" + p.Name + "([^a-z0-9_]+|$)", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.CultureInvariant));
#else
...
#pragma warning restore 0618
        ...
        internal static Action<IDbCommand, object> CreateParamInfoGenerator(Identity identity, bool checkForDuplicates, bool removeUnused, IList<LiteralToken> literals)
        {
            ...
            bool isStruct = TypeExtensions.IsValueType(type);
...
#if NET20
            var propsArr = Enumerable.ToArray(Enumerable.Where(type.GetProperties(), p => p.GetIndexParameters().Length == 0));
#else
...
#if NET20
            if (props == null) props = Enumerable.OrderBy(propsArr, x => x.Name);
#else
            ...
            foreach (var prop in props)
            {
                ...
                if (dbType == DynamicParameters.EnumerableMultiParameter)
                {
                    ...
                    if (TypeExtensions.IsValueType(prop.PropertyType))
                    {
...
#pragma warning restore 618
                    ...
                    if(prop != null)
                    {
                        ...
                        switch (typeCode)
                        {
                            ...
                            case TypeCode.Decimal:
                                ...
                                break;
                            default:
                                if (TypeExtensions.IsValueType(propType)) il.Emit(OpCodes.Box, propType); // command, sql, object value
...
#if NET20
        static readonly Dictionary<TypeCode, MethodInfo> toStrings = Enumerable.ToDictionary(new[]
        {
        ...
        }, x => TypeExtensions.GetTypeCode(x), x => TypeExtensions.GetPublicInstanceMethod(x, "ToString", new[] { typeof(IFormatProvider) }));
#else
        static readonly Dictionary<TypeCode, MethodInfo> toStrings = new[]
        {
            typeof(bool), typeof(sbyte), typeof(byte), typeof(ushort), typeof(short),
            typeof(uint), typeof(int), typeof(ulong), typeof(long), typeof(float), typeof(double), typeof(decimal)
        }.ToDictionary(x => Type.GetTypeCode(x), x => TypeExtensions.GetPublicInstanceMethod(x, "ToString", new[] { typeof(IFormatProvider) }));
#endif
        ...
        static readonly MethodInfo StringReplace = TypeExtensions.GetPublicInstanceMethod(typeof(string), "Replace", new Type[] { typeof(string), typeof(string) }),
...
#pragma warning restore 618
            ...
            if (TypeExtensions.IsEnum(effectiveType))
...
)
        {
...
#if NET20
            var names = Enumerable.ToArray(Enumerable.Select(Enumerable.Range(startBound, length), i => reader.GetName(i)));
#else
            ...
            else
            {
                ...
                if (explicitConstr != null)
                {
                    ...
                    {
                        if(!TypeExtensions.IsValueType(p.ParameterType))
                        {
...
#if NET20
            var members = Enumerable.ToList(specializedConstructor != null
                ? Enumerable.Select(names, n => typeMap.GetConstructorParameter(specializedConstructor, n))
                : Enumerable.Select(names, n => typeMap.GetMember(n)));
#else
            ...
            foreach (var item in members)
            {
                if (item != null)
                {
                    ...
                    else
                    {
                        ...
                        var unboxType = nullUnderlyingType != null && TypeExtensions.IsEnum(nullUnderlyingType) ? nullUnderlyingType : memberType;

                        if (TypeExtensions.IsEnum(unboxType))
                        {
...
#pragma warning restore 618
                    ...
                    {
                        il.Emit(OpCodes.Pop);
                        if (TypeExtensions.IsValueType(item.MemberType))
                        {
...
#if NET20
                return buffered ? Enumerable.ToList(result) : result;
#else
...
this
#endif
DataTable table, string typeName
#if !CSHARP30
            = null
#endif
            )
...
DataTable table)
        ...
        private static string __ToStringRecycle(
#if !NET20
this
#endif
StringBuilder obj)
...
#pragma warning restore 612, 618
        ...
        public IEnumerable<string> ParameterNames
        {
            ...
            {
#if NET20
                return Enumerable.Select(parameters, p => p.Key);
#else
        ...
        {
#if NET20
            foreach (var param in Enumerable.Select(parameters, p => p.Value))
#else
    ...
    sealed partial class DefaultTypeMap : SqlMapper.ITypeMap
    {
...
#if NET20
                   Enumerable.ToArray(Enumerable.Select(propertyInfo.GetIndexParameters(), p => p.ParameterType)),
#else
        ...
        {
#if NET20
            return Enumerable.ToList(t.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance));
#else
        ...
        /// <param name="types">DataReader column types</param>
        /// <returns>Matching constructor or default one</returns>
        internal static ConstructorInfo FindConstructor(Type type, string[] names, Type[] types)
        {
            var constructors = type.GetConstructors(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
#if NET20
            foreach (ConstructorInfo ctor in Enumerable.OrderByThenBy(constructors, c => c.IsPublic ? 0 : (c.IsPrivate ? 2 : 1), c => c.GetParameters().Length))
#else
...
#endif
            {
                ...
                for (; i < ctorParameters.Length; i++)
                {
                    ...
                    if (unboxedType != types[i]
                        && !(TypeExtensions.IsEnum(unboxedType) && Enum.GetUnderlyingType(unboxedType) == types[i])
                        && !(unboxedType == typeof(char) && types[i] == typeof(string))
                        && !(TypeExtensions.IsEnum(unboxedType) && types[i] == typeof(string)))
                    {
                        break;
                    }
                }
        ...
        {
            return FindConstructor(_type, names, types);
        }
...
#if NET20
            return new SimpleMemberMap(columnName, Enumerable.FirstOrDefault(parameters, p => string.Equals(p.Name, columnName, StringComparison.OrdinalIgnoreCase)));
#else
